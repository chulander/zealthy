...
export default async function OnboardingLayout({ children }: LayoutProps) {
  ...
  if (!workflowDetails) {
    console.log('No workflow details found, redirecting to /signup');
    redirect('/signup');
    return null;
  }
  ...
  return (
    <UserProvider value={{ userId, email, currentStep, components, workflowId, totalSteps, completedSteps }}>
